...
def solution(pattern, file_names):
    return [file_name for file_name in file_names if re.match(pattern, file_name)]
...
if __name__ == '__main__':
    with ReadContext() as r:
        ...
        for _ in range(C):
            W = r.input().strip()
            N = int(r.input().strip())
            FILE_NAMES = [r.input().strip() for _ in range(N)]

            regex = '^' + W.replace("?", ".").replace("*", ".*") + '$'
            answers = solution(pattern=re.compile(regex), file_names=FILE_NAMES)
